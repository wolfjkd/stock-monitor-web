...
用法示例:
  python price_alert.py --code sh600028 --target 2.90 --dir below
  python price_alert.py --code sz000001 --target 2.60 --dir above --interval 15
  python price_alert.py --config alert_config.json
  python price_alert.py --code sh600028 --target 2.87 --dir below --source tencent
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  # 单只：中国石化跌破2.90提醒（默认用通达信郑州节点，~15ms）
  python price_alert.py --code sh600028 --target 2.90 --dir below

  # 每3秒检查一次（配合tdx源极速响应）
  python price_alert.py --code sh600028 --target 2.87 --dir below --interval 3

  # 强制使用腾讯接口
  python price_alert.py --code sh600028 --target 2.90 --dir below --source tencent

  # 指定其他通达信服务器
  python price_alert.py --code sh600028 --target 2.90 --dir below --host 123.125.108.103:7709

  # 双向：工商银行跌破2.45或涨破2.55都提醒
  python price_alert.py --code sh601398 --target 2.45 --dir both
...
JSON配置文件格式:
{
  ...
  "alerts": [
    {"code": "sh600028", "name": "中国石化", "target": 2.90, "dir": "below"},
    {"code": "sz000001", "name": "平安银行", "target": 2.65, "dir": "above"}
  ]
}
        """
    )

    parser.add_argument('--code', type=str, help='股票代码 (如 sh600028)')
